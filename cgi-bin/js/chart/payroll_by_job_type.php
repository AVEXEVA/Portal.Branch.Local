...
function gd(year, month, day) {
    ...
    <?php 
        ...
        $job_result = sqlsrv_query($NEI,"
            ...
            WHERE TicketD.EDate >= '2017-03-30 00:00:00.000' AND TicketD.Total <= 24 AND TicketD.Total > 0 AND Job.Type <> 11 AND Job.Type <> 9 AND Job.Type <> 12
        ;");
        if($job_result){
            $Jobs = array();
            ...
            foreach($Jobs as $Job){
                ...
                $Division = $Job['Job_Type'];
                $Division = clean(preg_replace('/\s+/', '', $Division));
            ...
            foreach($data3 as $Division=>$array){
                ksort($array);
                $data = array();
                ?>var data_payroll_by_<?php echo $Division;?> = [<?php 
    ...
    var dataset_payroll_by_job_type = [<?php 
        $i = 0;
        foreach($data3 as $Division=>$array){
            $i++;
            ...
            ?>{
                ...
                data: data_payroll_by_<?php echo $Division;?>,
...
var options_payroll_by_job_type = {
    series: {
        shadowSize: 5
    },
    xaxes: [{
...
$(document).ready(function () {
    $.plot($("#flot-placeholder-payroll-by-job-type"), dataset_payroll_by_job_type, options_payroll_by_job_type);
    $("#flot-placeholder-payroll-by-job-type").UseTooltip();
});
