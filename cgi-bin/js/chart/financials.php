        ...
        <?php 
            ...
            foreach($dates_values as $date){
             ...
             $job_item_result = $database->query($Paradox,"
                ...
                FROM 
                    Paradox.dbo.JOBLABOR
                WHERE 
                    convert(date,[WEEK ENDING]) >= '2013-01-01 00:00:00.000'
                    AND ({$Jobs_SQL_Labor})
            ...
            if($job_item_result){
                while($array = sqlsrv_fetch_array($job_item_result)){
                    $date = substr($array['fDate'],0,10);
                    $dates[$date] = (isset($dates[$date])) ? $dates[$date] + floatval($array['Amount']) : floatval($array['Amount']);
                }
            }
            ksort($dates);
            ...
            if($invoice_result){
                while($array = sqlsrv_fetch_array($invoice_result)){
                    ...
                    $dates[$date] = (isset($dates[$date])) ? $dates[$date] - ($array['Amount'] * (1 - $Overhead)) : 0 - ($array['Amount'] * (1 - $Overhead));
                }
            }
            ksort($dates);
            $total = 0;
