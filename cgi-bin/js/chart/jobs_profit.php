...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($_SESSION['Branch']) || $_SESSION['Branch'] == 'Nouveau Elevator'){
        ...
        $Privileged = FALSE;
        if(isset($My_Privileges['Finances']) && $My_Privileges['Finances']['User_Privilege'] >= 4 && $My_Privileges['Finances']['Group_Privilege'] >= 4 && $My_Privileges['Finances']['Other_Privilege'] >= 4){$Privileged = TRUE;}
    }
    if(!isset($array['ID']) || !$Privileged){ }
    else {
$Random_Graph_ID = rand(1,9999999);
?><div class="flot-chart"><div class="flot-chart-content" id="flot-placeholder-<?php echo $Random_Graph_ID;?>-graph"></div></div><script>
    ...
    var a_<?php echo $Random_Graph_ID;?>_data = [<?php 
            ...
            if(count($Jobs) > 0){
                ...
                if($Check){
                    $tJobs = array();
                    $aJobs = array();
                    foreach($Jobs as $Job){$tJobs[] = "Job.ID = '{$Job}'";$aJobs[] = "[JOBLABOR].[JOB #]='{$Job}'";}
                    $Jobs = implode(" OR ",$tJobs);
                    ...
                    $job_item_result = $database->query(null,"
                        SELECT 
                            JobI.fDate as fDate,
                            JobI.Amount as Amount
                        FROM JobI LEFT JOIN nei.dbo.Job ON JobI.Job = Job.ID
                        WHERE 
                            ({$Jobs})
                            AND JobI.Type='1'
                            AND JobI.fDate >= '2017-03-30 00:00:00.000'
