...
function post_time_allocation(link){
  ...
  if(isCanvasBlank($("canvas#signature"))){message = message + "A Signature needs to be filled out. ";}
...
function saveTicket(/*position*/){
  ...
  $.ajax({
    ...
    method:"POST",
    success:function(code){
      <?php if($_SESSION['User'] == 895 && FALSE){?>if(confirm("Can you please add information pertaining to the controller?")){
