...
function time_on_site(/*position*/){
  ...
  //if((loc_lat == 0) || (loc_lng == 0) || distance(position.coords.latitude, position.coords.longitude, loc_lat, loc_lng, 'M') <= .42){
    $.ajax({
      url:"cgi-bin/php/post/ticket_time_on_site.php",
      data: {
        ID : '<?php echo $_GET['ID'];?>'/*,
...
function post_time_on_site(link){
  ...
  if (navigator.geolocation) {
      //navigator.geolocation.getCurrentPosition(time_on_site, showError, {enableHighAccuracy:true, timeout:10000});
      time_on_site();
  } else {
...
function time_en_route(position){
  ...
  $("button#on_site").attr('disabled','disabled');
  $.ajax({
    url:"cgi-bin/php/post/ticket_time_en_route.php",
    cache:false,
    data: {
      ID        : '<?php echo $_GET['ID'];?>',
      Latitude  : position.coords.latitude,
...
function reset_time(link){
  if (confirm('Are you sure you want to reset the ticket? All times will be reset to null.')) {
    $.ajax({
      url:"cgi-bin/php/post/ticket_time_reset.php",
...
function post_time_completed(link){
  $.ajax({
    url:"cgi-bin/php/post/ticket_time_completed.php",
...
function saveTicket(position){
  ...
  $.ajax({
    url:"cgi-bin/php/post/save_ticket.php",
    cache: false,
    processData: false,
    contentType: false,
    data: ticketData,
...
function save_internal_comments(link){
  ...
  $(link).attr('disabled','disabled');
  $.ajax({
    url:"cgi-bin/php/post/ticket_internal_comments.php",
    data: {
      ID:  '<?php echo $_GET['ID'];?>',
...
function showPosition(position) {
    $.ajax({
      ...
      url:"cgi-bin/php/post/ticket-gps.php",
      method:"POST",
      success:function(code){}
    });
}
...
/*function watchSuccess(position){
  ...
  gpsData.append("ID",<?php echo isset($_GET['ID']) ? $_GET['ID'] : NULL;?>);
...
<?php if($Ticket['Assigned'] == 3){?>$(document).ready(function(){
    navigator.geolocation.watchPosition(watchSuccess, showError, {enableHighAccuracy:true, timeout:10000});
});<?php }?>*/
