		...
		if(isset($_POST['action']) && $_POST['action'] == 'edit'){
			if(isset($_POST['data']) && count($_POST['data']) > 0){
				...
				foreach($_POST['data'] as $ID=>$Location){
					$r = $database->query(null,"SELECT Route.ID AS ID FROM nei.dbo.Route WHERE Route.Name = ?;",array($Location['Route']));
					if($r){$Route_ID = sqlsrv_fetch_Array($r)['ID'];}
					$r = $database->query(null,"SELECT Zone.ID  AS ID FROM nei.dbo.Zone WHERE Zone.Name = ?;",array($Location['Division']));
			...
			if(isset($_POST['data']) && count($_POST['data']) > 0){
				foreach($_POST['data'] as $ID=>$Location){
					...
					$r = $database->query(null,"SELECT Zone.ID  AS ID FROM nei.dbo.Zone WHERE Zone.Name = ?;",array($Location['Division']));
					if($r){$Division_ID = sqlsrv_fetch_Array($r)['ID'];}
					$r = $database->query(null,"SELECT Terr.ID  AS ID FROM nei.dbo.Terr WHERE Terr.Name = ?;",array($Location['Territory']));
