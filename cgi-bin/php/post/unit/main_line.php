...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
  ...
  if(!isset($array['ID'])  || !$Privileged){?><html><head>
    ...
    if(isset($_POST['ID']) && !isset($_POST['Item'])){
      //INSERT BLANK RECORD INTO Item, Product & Main_Line
      //Insert into Product
      $sQuery =
      ...
      $sQuery =
        " INSERT INTO Device.dbo.[Main_Line](Item, Voltage, Amperage, Maintained_By, Locked_Out, Tagged_Out)
          VALUES(?, ?, ?, ?, ?, ?);";
      $params = array(
        $Item_ID,
        $_POST['Voltage'],
      ...
      //Update Main_Line
      $sQuery =
        " UPDATE  Device.dbo.[Main_Line]
          SET     Main_Line.Voltage = ?,
                  Main_Line.Amperage = ?,
                  Main_Line.Maintained_By = ?,
                  Main_Line.Locked_Out = ?,
                  Main_Line.Tagged_Out = ?
          WHERE   Main_Line.Item = ?;";
      $params = array(
        $_POST['Voltage'],
        $_POST['Amperage'],
        $_POST['Maintained_By'],
