...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
  ...
  if(!isset($array['ID'])  || !$Privileged){?><html><head>
    ...
    if(isset($_POST['ID']) && !isset($_POST['Item'])){
      //INSERT BLANK RECORD INTO Item, Product & Edge
      //Insert into Product
      $sQuery =
      ...
      if(isset($_FILES['Image']) && file_exists($_FILES['Image']['tmp_name'])){
        $sQuery =
          " INSERT INTO Device.dbo.Item(Type, Product, Device, [Serial], [Condition], Notes, Image, Image_Type, Height, Vendor_Purchase_Order, Blueprint)
            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?);";
        $params = array(
          'Edge',
          $Product_ID,
        ...
        $sQuery =
          " INSERT INTO Device.dbo.Item(Type, Product, Device, [Serial], [Condition], Notes, Height, Vendor_Purchase_Order, Blueprint)
            VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?);";
        $params = array(
          'Edge',
          $Product_ID,
      ...
      //Insert into Edge
      $sQuery =
        " INSERT INTO Device.dbo.[Edge](Item, Beams, Beam_Seperation, LED_Indicators, Nudging, Infared_Pulse_Ratio)
          VALUES(?, ?, ?, ?, ?, ?);";

      $params = array(
        $Item_ID,
        $_POST['Beams'],
        $_POST['Beam_Seperation'],
        $_POST['LED_Indicators'],
        $_POST['Nudging'],
        $_POST['Infared_Pulse_Ratio']
      );
      $r = sqlsrv_query($database_Device, $sQuery, $params);
      ...
      //Update Edge
      $sQuery =
        " UPDATE  Device.dbo.[Edge]
          SET     Edge.Beams = ?,
                  Edge.Beam_Seperation = ?,
                  Edge.LED_Indicators = ?,
                  Edge.Nudgable = ?,
                  Edge.Nudging = ?,
                  Edge.Infared_Pulse_Ratio = ?
          WHERE   Edge.Item = ?";
      $params = array(
        $_POST['Beams'],
        $_POST['Beam_Seperation'],
