...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
  ...
  if(!isset($array['ID'])  || !$Privileged){
    ...
    if(isset($_POST['ID']) && !isset($_POST['Item'])){
      //INSERT BLANK RECORD INTO Item, Product & Buffer
      //Insert into Product
      $sQuery =
      ...
      //Insert into Buffer
      $sQuery =
        " INSERT INTO Device.dbo.[Buffer](Item, Type, Stroke_Range, Minimum_Load, Maximum_Load, Maximum_Car_Speed)
          VALUES(?, ?, ?, ?, ?, ?);";
      $params = array(
        $Item_ID,
        $_POST['Type'],
      ...
      //Update Buffer
      $sQuery =
        " UPDATE  Device.dbo.Buffer
          SET     Buffer.Type = ?,
                  Buffer.Stroke_Range = ?,
                  Buffer.Minimum_Load = ?,
                  Buffer.Maximum_Load = ?,
                  Buffer.Maximum_Car_Speed = ?
          WHERE   Buffer.Item = ?";
      $params = array(
        $_POST['Type'],
        $_POST['Stroke_Range'],
