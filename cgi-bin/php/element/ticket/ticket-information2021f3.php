    ...
    if($r){
      ...
      if(is_array($Ticket)){
        ...
        if(isset($Ticket['ID']) && is_numeric($Ticket['ID']) && isset($Ticket['WorkOrder']) && ($Ticket['WorkOrder'] == null || $Ticket['WorkOrder'] == 0 || $Ticket['WorkOrder'] == '')){
          $database->query(null,"UPDATE TicketO SET TicketO.WorkOrder = ? WHERE TicketO.ID = ?",array($_GET['ID'], $_GET['ID']));
...
<div class="panel panel-primary">
  <div class='panel-heading' style='padding:1px;padding-left:10px !important;margin:0px;margin-bottom:5px;'><h4><?php \singleton\fontawesome::getInstance( )->Info(1);?> Description</h4></div>
    ...
    <div class='row'>
      ...
      function changeUnit(link){
        $.ajax({
          ...
          success:function(code){document.location.href='ticket.php?ID=<?php echo $_GET['ID'];?>';}
        });
      }
    ...
    <div class='row'>
      <div class='col-xs-12'><button onClick="document.location.href='ticket.php?ID=<?php echo $_GET['ID'];?>&Edit=True';" style='width:100%;'>Edit Ticket</button></div>
    </div>
    <?php }?>
  </div>
  <?php
  ...
  <div class='panel-body'>
    ...
    <?php $r = $database->query(null,"SELECT * FROM (
      ...
      UNION ALL
      (SELECT TicketDArchive.ID, TicketDArchive.WorkOrder  FROM TicketDArchive WHERE TicketDArchive.WorkOrder = ? AND TicketDArchive.WorkOrder <> 0)
    ) AS Tickets WHERE Tickets.ID <> ?;",array($Ticket['WorkOrder'],$Ticket['WorkOrder'],$Ticket['WorkOrder'],$_GET['ID']));
    $i = 1;
    if($r){while($row = sqlsrv_fetch_array($r)){
      ?><div class='row'><div class='col-xs-4' style='text-align:right;'><?php echo $i;?>.</div><div class='col-xs-8' onClick="document.location.href='ticket.php?ID=<?php echo $row['ID'];?>';">Ticket #<?php echo $row['ID'];?></div></div><?php
      $i++;
    }}?>
    <div class='row'><div class='col-xs-12'>&nbsp;</div></div>
  </div>
  ...
  <div class='panel-body' style='padding:10px;'>
    ...
    <div class='row'>
    	<div class='col-xs-4'><?php \singleton\fontawesome::getInstance( )->Timesheet(1);?> 4 Day Work Week</div>
    	<div class='col-xs-8'><input type='checkbox' onChange='toggleRegularHours();' />
    </div>
  ...
  <div class='panel-body' style='padding:10px;'>
    ...
    <div class='row resolution'>
      <div class='col-xs-12'><i class="fa fa-paragraph fa-1x fa-fw" aria-hidden="true"></i> Resolution:</div>
      <div class='col-xs-12'><select name='Resolution_Code' multiple style='width:100%;' size='10'>
        <?php
          $r = $database->query($Portal, "SELECT * FROM Portal.dbo.Resolution_Code ORDER BY CONVERT( VARCHAR(50) , [Resolution_Code].[Description] ) ASC;");
          $Resolution_Codes = array();
          while($row = sqlsrv_fetch_array($r)){
            $s = str_pad($row['Name'], 10, " ") . ' - ' . $row['Description'];
            $s = str_replace(' ', '&nbsp;', $s);
            $Resolution_Codes[] = $s;

          }
          foreach($Resolution_Codes as $Resolution_Code){?><option style='font-family:monospace;' value='<?php echo str_replace('&nbsp;', ' ', $Resolution_Code);?>'><?php echo $Resolution_Code;?></option><?php 
        }?>
      </select></div>
      <div class='col-xs-12'>&nbsp;</div>
      <div class='col-xs-6'><button onClick='addResolutionCode();'>Add Resolution Code</button></div>
      <div class='col-xs-6'><button onClick='clear_Resolution();'>Clear Resolution</button></div>
    </div>
    <div class='row'>
      <div class='col-xs-12'>&nbsp;</div>
      <div class='col-xs-12'><textarea disabled id='TextareaResolution' name='Resolution' style='width:100%;font-family:monospace;color:black !important;' rows='5' placeholder='Resolution Code(s)'><?php 
        $strrpos = strrpos($Ticket2['DescRes'], '-----Notes-----');
        $strpos2 = strrpos($Ticket2['DescRes'], '-----Codes-----');
        echo substr($Ticket2['DescRes'], $strpos2 + strlen('-----Codes-----'), $strrpos - $strpos2 - strlen('-----Codes-----'));
      ?></textarea></div>
      <div class='col-xs-12'>&nbsp;</div>
    </div>
    <div class='row'>
      <div class='col-xs-12'>&nbsp;</div>
      <div class='col-xs-12'><textarea id='TextareaResolution2' name='Resolution2' style='width:100%;font-family:monospace;color:black !important;' rows='5' placeholder='Required Resolution Description'><?php 
        $strrpos = strrpos($Ticket2['DescRes'], '-----Notes-----');
        echo substr($Ticket2['DescRes'], $strrpos + strlen('-----Notes-----'));
      ?></textarea></div>
      <div class='col-xs-12'>&nbsp;</div>
    </div>
  ...
  <div class='panel-body' style='padding:10px;'>
    ...
    <div class='signature row'>
      ...
      <div class='col-xs-8'><input type='text' placeholder='Required Signee Name' name='Signature_Name' style='width:100%;' value='<?php echo $Ticket2['SignatureText'];?>' /></div>
  ...
  <div class='panel-body' style='padding:10px;font-size:14px;'>
    ...
    <div class='row expenses'>
      <div class='col-xs-4'><i class="fa fa-paperclip fa-1x fa-fw" aria-hidden="true"></i> Photo:</div>
      <?php
        $r = $database->query(null,"SELECT [TicketPic].[PicData] FROM TicketPic WHERE [TicketPic].TicketID = ? AND [TicketPic].PictureName LIKE '%TCK%';",array($_GET['ID']));
        $i = 0;
        if($r){while($row = sqlsrv_fetch_array($r)){
          $row['Type'] = 'image/jpeg';
  ...
  <div class='panel-body' style='padding:10px;font-size:14px;'>
    <div class='email row'>
      <div class='col-xs-4'><?php \singleton\fontawesome::getInstance( )->Email(1);?> Email:</div>
      <div class='col-xs-8'><?php
      $r = $database->query(null,"SELECT * FROM Ticket_Email WHERE Ticket_Email.Ticket = ?;",array($_GET['ID']));
      $i = 0;
      if($r){while($row = sqlsrv_fetch_array($r)){
  ...
  <div class='panel-body' style='padding:10px;'>
    ...
    <div class='row'>
    	<div class='col-xs-4'><?php \singleton\fontawesome::getInstance( )->Timesheet(1);?> 4 Day W.W.</div>
    	<div class='col-xs-8'><input type='checkbox' onChange='toggleRegularHours();' /></div>
    </div>
    <div class='row timesheet'>
      <div class='col-xs-4'><?php \singleton\fontawesome::getInstance( )->Timesheet(1);?> Regular:</div>
  ...
  <div class='panel-body' style='padding:10px;'>
    ...
    <div class='row'>
      ...
      if((isset($Ticket['TimeRoute']) && $Ticket['TimeRoute'] == '1899-12-30 00:00:00.000') || !isset($Ticket['TimeRoute']) || $Ticket['TimeRoute'] == NULL){?>
        ...
        $r = $database->query(null,"SELECT * FROM TicketDPDA LEFT JOIN Emp ON Emp.fWork = TicketDPDA.fWork WHERE Emp.ID = ? AND ( TicketDPDA.Comments = '' OR TicketDPDA.Comments IS NULL );",array($_SESSION['User']));
        if($r && is_array(sqlsrv_fetch_array($r))){$disabled3 = true;}
        else{$disabled3 = false;}
        ?>
      <div class='panel-heading' style='padding:1px;padding-left:10px !important;margin:0px;margin-bottom:5px;'><h4><?php \singleton\fontawesome::getInstance( )->Timesheet(1);?> Time</h4></div>
      <div class='panel-body' style='padding-top:10px;'>
        <div class='row'>
            ...
            } elseif($disabled3 && false) {
              echo 'onClick="alert(\'You have to fill out mechanic notes on previous ticket(s).\');"';
            } else {
...
<script>
    function addResolutionCode(){
      var nResolutions = $("select[name='Resolution_Code']").val().toString().split(',');
      var oResolutions = $("#TextareaResolution").val().toString().split('\n');
      var Resolutions = [];
      $('#TextareaResolution').html('');
      for( i in oResolutions ){
        if( oResolutions[i] == '' ){continue;}
        Resolutions.push( oResolutions[i] );
      }
      for( i in nResolutions ){
        if( Resolutions.includes( nResolutions[ i ] ) ){continue;}
        Resolutions.push( nResolutions[i] );
        for(v in nResolutions){<?php
          if($r){while( $row = sqlsrv_fetch_array( $r ) ){
            ?>if(nResolutions[v] == '<?php echo $row['Name'] . ' - ' . $row['Description'];?>'){ resolutionType( nResolutions[v], '<?php echo $row['Question'];?>', '<?php echo $row['Type'];?>' );}
          <?php
          $rQuestions[] = $row;
          }}
        ?>}
      }
      $("#TextareaResolution").val( Resolutions.join('\n') );
    }
    function clear_Resolution(){$('#TextareaResolution').val('');}
</script>
<?php require("../../../../cgi-bin/js/pages/ticket2021f3.php");?>
<style>
