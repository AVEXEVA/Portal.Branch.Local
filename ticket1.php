...
if(isset( $_SESSION['User'], $_SESSION['Hash' ] ) ){
    ...
    elseif($Privileges['Ticket']['Group_Privilege'] >= 4){
        ...
        if(!is_null($Location)){
            $r = \singleton\database::getInstance( )->query(  null,"SELECT ID FROM TicketO WHERE TicketO.LID='{$Location}' AND fWork='{$User['fWork']}'");
            $r2 = \singleton\database::getInstance( )->query( null,"SELECT ID FROM TicketD WHERE TicketD.Loc='{$Location}' AND fWork='{$User['fWork']}'");
            $r3 = \singleton\database::getInstance( )->query( null,"SELECT ID FROM TicketDArchive WHERE TicketDArchive.Loc='{$Location}' AND fWork='{$User['fWork']}'");
            if($r || $r2 || $r3){
    ...
    if(!isset($array['ID'])  || !$Privileged){?><html><head></head></html><?php }
        ...
        \singleton\database::getInstance( )->query($Portal,"INSERT INTO Activity([User], [Date], [Page]) VALUES(?,?,?);",array($_SESSION['User'],date("Y-m-d H:i:s"), "ticket.php?ID={$_GET['ID']}"));
$Ticket = null;
...
$r = \singleton\database::getInstance( )->query(null,"SELECT PDATicketSignature.Signature AS Signature FROM PDATicketSignature WHERE PDATicketSignature.PDATicketID = ?",array($_GET['ID']));
    ...
    <div id="wrapper" class="<?php echo isset($_SESSION['Toggle_Menu']) ? $_SESSION['Toggle_Menu'] : null;?>;">
        ...
        <div id="page-wrapper" class='content' style='overflow-x:hidden;<?php if(isset($_SESSION['Branch']) && $_SESSION['Branch'] == 'Customer'){?>margin:0px !important;<?php }?>'>
            ...
            <div class='Screen-Tabs shadower'>
                <div class='row'>
                    <div class='Home-Screen-Option col-lg-1 col-md-2 col-xs-3' onClick="someFunction(this,'ticket-information2021f3.php?ID=<?php echo $_GET['ID'];?><?php if(isset($_GET['Edit'])){?>&Edit=True<?php }?><?php if(isset($_GET['Ticket_Update'])){?>&Ticket_Update=1<?php }?>');">
