...
function gd(year, month, day) {
    ...
    <?php 
        ...
        $job_result = $database->query(null,"
            SELECT Loc.Zone AS Division, 
				   ((TicketD.Reg * PRWage.Reg) + (TicketD.OT * PRWage.OT1) + (TicketD.DT * PRWage.OT2) + (TicketD.TT * PRWage.Reg)) AS Total, 
        ...
        if($job_result){
            ...
            foreach($data3 as $Division=>$array){
                ksort($array);
                $data = array();
                ?>var data_maintenance_hours_payroll_by_<?php echo $Division;?> = [<?php 
    ...
    var dataset_maintenance_hours_payroll_by_division = [<?php 
        $i = 0;
        foreach($data3 as $Division=>$array){
            $i++;
            ...
            ?>{
                ...
                data: data_maintenance_hours_payroll_by_<?php echo $Division;?>,
...
var options_maintenance_hours_payroll_by_division = {
    series: {
        shadowSize: 5
    },
    xaxes: [{
...
$(document).ready(function () {
    $.plot($("#flot-placeholder-maintenance-hours-payroll-by-division"), dataset_maintenance_hours_payroll_by_division, options_maintenance_hours_payroll_by_division);
    $("#flot-placeholder-maintenance-hours-payroll-by-division").UseTooltip();
});
