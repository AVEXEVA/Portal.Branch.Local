...
<body onload='finishLoadingPage();'>
  ...
  <script>
	...
	$(Editor_Tickets.field('Location').node()).on('change',function(){
		...
		$.ajax({
			url:'bin/php/get/Location_by_Name.php',
			method:"GET",
			dataType: "json",
			data: { Name : dName},
			success:function( code ){
				...
				$.ajax({
					url:'bin/php/get/Unit_States_by_Location.php?ID=' + Location.data[0].ID,
					method:"GET",
					dataType:"JSON",
					success:function(code){
	...
	$(Editor_Tickets.field('Unit_State').node()).on('change',function(){
		...
		$.ajax({
			url:'bin/php/get/Unit_by_State.php',
			method:"GET",
			dataType: "json",
			data: { State : dState},
			success:function( code ){
	...
	var Table_Tickets = $('#Table_Tickets').DataTable( {
		"ajax": {
			"url":"bin/php/reports/Active_Tickets.php",
