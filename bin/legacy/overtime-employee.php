                    ...
                    <div class="panel panel-primary">
                        ...
                        <div class='panel-heading'>
                          <form>
                              ...
                              <div class='row'>
                                <div class='col-xs-1' style='text-align:right;'>Employee:</div>
                                <div class='col-xs-11'><select name='Supervisor' style='color:black;'><option value=''>Select</option>
                                  <?php
                                    $r = $database->query(null,"SELECT Emp.ID AS ID, Emp.Last + ', ' + Emp.fFirst AS Name FROM Emp WHERE Emp.Field = 1 AND Emp.Status = 0 ORDER BY Emp.Last ASC ;");
                                    if($r){while($row = sqlsrv_fetch_array($r)){?><option value='<?php echo $row['ID'];?>' <?php echo isset($_GET['Supervisor']) && $row['ID'] == $_GET['Supervisor']  && $_GET['Supervisor'] != '' ? 'selected' : '';?>><?php echo $row['Name'];?></option><?php }}?>
                                </select></div>
                              </div>
                              <div class='row'>
                                            ...
                                            $sQuery = " SELECT  Tickets.Custom1 AS Supervisor,
                                                ...
                                                WHERE   Tickets.EDate >= ?
                                                        ...
                                                        AND Tickets.Job IN (
                                                          ...
                                                          WHERE (Job.Type = ? {$job_type}) AND (Emp.ID = ? {$supervisor}) AND TicketD.EDate >= ? AND TicketD.EDate <= ? AND Job.fDesc LIKE '%' + ? + '%'
                                                          GROUP BY Job.ID
                                                          HAVING {$showRegular2}
                                                        )
                                                GROUP BY Tickets.Custom1, Tickets.Tag, Tickets.fDesc, Tickets.fFirst, Tickets.Last, Tickets.Job, Tickets.Type, Tickets.Title, Tickets.Status, Tickets.Emp, Tickets.Custom20, Tickets.Employee_Supervisor
                                                HAVING  (Tickets.Type=? {$job_type}) AND (Tickets.Emp = ? {$supervisor}) AND Tickets.Type <> 9 AND Tickets.Type <> 12 AND Tickets.fDesc LIKE '%' + ? + '%' {$showRegular};
                                            ;";
                                            //echo $sQuery;
                                                ...
                                                foreach($arrays0 as $location=>$arrays){
                                                    ...
                                                    foreach($arrays as $job=>$arrayed){
                                                      ...
                                                      foreach($arrayed as $array){
                                                          ...
                                                          $total_total += $reg_total + $ot_total + $dt_total + $nt_total;
                                                          ?><tr><td></td><td><?php echo $array['Title'];?></td><td><?php echo $array['fFirst'] . ' ' . $array['Last'];?></td><td><?php echo $array['Employee_Supervisor'];?></td><td><?php echo $array['SumOfReg'];?></td><td><?php echo $array['SumOfNT'];?></td><td><?php echo $array['SumOfOT'];?></td><td><?php echo $array['SumOfDT'];?></td><td><?php echo $array['SumOfTotal'];?></td></tr><?php
                                                      }
