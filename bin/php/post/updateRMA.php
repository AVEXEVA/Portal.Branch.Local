...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!$Privileged || count($_POST) == 0){?><html><head><script>document.location.href='../login.php';</script></head></html><?php }
        ...
        var_dump($_POST);
        $database->query($Portal,"UPDATE RMA SET Name=?, Date=?, Address=?, RMA=?, Received=?, Returned=?, Tracking=?, PO = ?, Link = ?, Description = ?, Status = ? WHERE ID=?;",array($_POST['Name'],$_POST['Date'],$_POST['Address'],$_POST['RMA'],$_POST['Received'],$_POST['Returned'],$_POST['Tracking'],$_POST['PO'],$_POST['Link'],$_POST['Description'],$_POST['Status'],$_POST['ID']));
