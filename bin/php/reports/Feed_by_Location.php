...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    $r = $database->query(null,"SELECT * FROM nei.dbo.Emp WHERE ID = ?",array($_SESSION['User']));
	...
	elseif($My_Privileges['Location']['Owner'] >= 4 && is_numeric($_GET['ID'])){
		$r  = $database->query( null,"SELECT * FROM nei.dbo.TicketO        WHERE TicketO.LID        = ? AND fWork=?;",array($_GET['ID'],$My_User['fWork']));
		$r2 = $database->query( null,"SELECT * FROM nei.dbo.TicketD        WHERE TicketD.Loc        = ? AND fWork=?;",array($_GET['ID'],$My_User['fWork']));
		$r3 = $database->query( null,"SELECT * FROM nei.dbo.TicketDArchive WHERE TicketDArchive.Loc = ? AND fWork=?;",array($_GET['ID'],$My_User['fWork']));
