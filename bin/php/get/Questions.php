  ...
  //Connection
    ...
    $sQuery =
      ...
      $rResult = $database->query(
        ...
        $_SESSION[ 'Tables' ][ 'Questions' ] = isset( $_SESSION[ 'Tables' ][ 'Questions' ]  ) ? $_SESSION[ 'Tables' ][ 'Questions' ] : array( );
        if( count( $_SESSION[ 'Tables' ][ 'Questions' ] ) > 0 ){ foreach( $_SESSION[ 'Tables' ][ 'Questions' ] as &$Value ){ $Value = false; } }
        $_SESSION[ 'Tables' ][ 'Questions' ][ 0 ] = $_GET;
        while( $Row = sqlsrv_fetch_array( $fResult ) ){
            $_SESSION[ 'Tables' ][ 'Questions' ][ $Row[ 'ID' ] ] = true;
            $iFilteredTotal++;
        }

        $parameters = array( );
        $sQuery = " SELECT  COUNT( [Question].[ID] )
                FROM    [Question];";
