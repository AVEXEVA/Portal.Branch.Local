...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($_SESSION['Branch']) || $_SESSION['Branch'] == 'Nouveau Elevator'){
        ...
        $Privileged = FALSE;
        if(isset($My_Privileges['Admin']) && $My_Privileges['Admin']['Owner'] >= 4 && $My_Privileges['Admin']['Group'] >= 4 && $My_Privileges['Admin']['Other'] >= 4){
        	$Privileged = TRUE;
		}
    }
    //
    if(!isset($array['ID']) || !$Privileged){?><html><head><script>document.location.href="../login.php?Forward=admin.php";</script></head></html><?php }
		...
		$database->query($Portal,"INSERT INTO Activity([User], [Date], [Page]) VALUES(?,?,?);",array($_SESSION['User'],date("Y-m-d H:i:s"), "admin.php"));
					...
					<div class="panel panel-primary">
						...
						<div class="panel-body BankGothic white-background shadow">
							...
							var Table_Activity = $('#Table_Activity').DataTable( {
								"ajax": {
									"url":"bin/php/reports/Recent_Activity.php",
