...
if(isset($_SESSION['User'],$_SESSION['Hash'])){
    ...
    if(!isset($_SESSION['Branch']) || $_SESSION['Branch'] == 'Nouveau Elevator'){
        ...
        $Privileged = FALSE;
        if(isset($My_Privileges['Customer']) && $My_Privileges['Customer']['Other'] >= 4){$Privileged = TRUE;}
    }
    $database->query($Portal,"INSERT INTO Activity([User], [Date], [Page]) VALUES(?,?,?);",array($_SESSION['User'],date("Y-m-d H:i:s"), "customer-modernization.php"));
...
var Table_Active_Modernizations = $('#Table_Active_Modernizations').DataTable( {
	"ajax": "bin/php/reports/Active_Modernizations_by_Customer.php?ID=<?php echo $_GET['ID'];?>",
	"columns": [
		{ "data": "ID"},
		{ "data": "Name" },
...
<div class="panel panel-primary">
	...
	<script>
		var Table_Modernization_Job_Items = $('#Table_Modernization_Job_Items').DataTable( {
			"ajax": "bin/php/reports/Modernization_Job_Items_by_Customer.php?ID=<?php echo $_GET['ID'];?>",
			"columns": [
				{ "data": "Job"},
				{ "data": "Item" },
				{ "data": "Date",render: function(data){return data.substr(5,2) + "/" + data.substr(8,2) + "/" + data.substr(0,4);}}
